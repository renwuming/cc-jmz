...
cc.Class({
  ...
  changeStep(step) {
    ...
    this.bottomTables.forEach(t => {
      if (
        t.node.stepFlag === this.stepFlag &&
        t.node.menuTag === this.activeMenu
      ) {
        t.node.active = true
      } else {
        t.node.active = false
  ...
  onLoad() {
    ...
    this.HomeBtn.node.getChildByName('Background').color =
      Global.currentStyle.bkColor
    ...
    this.bottomTables = [this.DatiTableE, this.DatiTableF]
    this.changeMenu()
    this.initTip()

    this.getGameData()

    this.HomeBtn.node.on(
      'click',
      function() {
    ...
    historyData.forEach(battle => {
      ...
      if (teamIndex === desTeam) {
        ...
        codes.forEach((code, index) => {
          ...
          if (!tableData1[EnemyAnswer])
            tableData1[EnemyAnswer] = {
              rightList: [],
              wrongList: []
            }
          if (EnemyAnswer === code) {
    ...
    Global.Get({
      ...
      if (userIndex === desUserIndex) {
        ...
        if (question) {
          ...
          if (this.teamIndex === desTeam && !answerF) {
            // 若为队友出题
            this.updateDatiTableF(question)
            this.changeStep(1)
          } else if (
            this.teamIndex !== desTeam &&
            !answerE &&
            userIndex === eIndex &&
            activeBattle > 1
          ) {
      ...
      this.HistoryTable.getComponent('HistoryTable').init(
        history,
        sumList,
        teamNames
      )
